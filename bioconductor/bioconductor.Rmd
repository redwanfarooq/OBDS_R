...
> Answer: Vignettes describe typical uses of the package, reference manual entries describe individual functions within the package
...
On the teaching cluster, all the packages required for the course are already installed. As a result, you do not need (and should not) install any package yourself.
...
The package `r BiocStyle::CRANpkg("BiocManager")` is the only official Bioconductor package on CRAN. It provides functions to install, update, and more generally manage your library of Bioconductor (and CRAN) packages.
...
> Answer: Lists attached packages and flags packages which are out-of-date or too new for the current version of Bioconductor
...
```{r}
truseq_adapters <- readDNAStringSet("TruSeq3-PE-2.fa", format = "fasta")
truseq_adapters
```
...
```{r}
sub <- subset(gtf_ACTB, type == "transcript")
length(sub)
sum(duplicated(sub$transcript_id)) # Check to ensure no duplicated transcript IDs
```
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys = human_symbols, keytype = "SYMBOL", columns = c("ENTREZID", "ENSEMBL"))
```

-   What happens if you do not specify `AnnotationDbi`?

> Answer: Still works, but there is a theoretical risk of using `select()` function from another package (e.g. `dplyr`)

-   Use the `mapIds()` method to get the Ensembl identifier for those gene symbols.

```{r}
mapIds(org.Hs.eg.db, keys = human_symbols, keytype = "SYMBOL", column = "ENSEMBL")
```
...
> Answer: Specify database version
...
-   Use again the `useMart()` function, and replace the `mart` object by a new one that points to information for *Homo sapiens*.
...
> Answer: Self-explanatory function names
...
```{r}
assay_counts <- read.csv("summarized_experiment/counts.csv", row.names = 1) # Relative path to symlink to CSV file in project directory
assay_counts
```
...
```{r}
sample_metadata <- read.csv("summarized_experiment/sample_metadata.csv", row.names = 1) # Relative path to symlink to CSV file in project directory
sample_metadata
```

```{r}
gene_metadata <- read.csv("summarized_experiment/gene_metadata.csv", row.names = 1) # Relative path to symlink to CSV file in project directory
gene_metadata
```
...
```{r}
setdiff(slotNames(demo_deseq), slotNames(demo_se))
```
...
> Answer: All components have accessor functions; only `rowRanges()` accessor has a method for `SummarisedExperiment` class
...
setdiff(slotNames(demo_sce), slotNames(demo_se))
```
...
> Answer: Self-explanatory functions
...
> Answer: Biocunductor repositories added to global `repos`; therefore it should now be possible to install Bioconductor packages using `install.packages()`
